...
public class CameraResolution : MonoBehaviour {
	...
	public List<string> imgNameList = new List<string>(); //이미지파일 이름만 
	...
	/// 경품 이미지 이름과 개수
	private	string path2 = @"c:\info\img_item.txt";
	/// 경품 이미지 저장폴더
	private	string path3 = @"c:\info\img\";
    ...
    void Awake () {
		...
		try {
			...
			errorPopup.SetActive(true);
			errorText.text = "c:/info/img_item.txt 파일이 없습니다";
			return;
		}
	...
	private void getItem() {
		...
		for (int i = 0; i < itemValue.Length; i++){
			...
			if (glassinfo != null) {
				...
				imgNameList.Add(glassinfo[0]); //이미지 이름만 저장 path 가져올때 사용하려고
	...
	void getImg() {
		...
		for (int i = 0; i < imgNameList.Count; i++) {
			...
			try {
				...
				byte[] b = System.IO.File.ReadAllBytes(path3 + imgNameList[i]);
				Texture2D texture2 = new Texture2D(0,0);
				texture2.LoadImage(b);
				spriteList.Add( Sprite.Create(texture2, new Rect(0,0,texture2.width,texture2.height), new Vector2(0.5f, 0.5f)) );
				...
				errorCheck = true;
				// Debug.Log("EEEEEEE");
				errorImgName.Add(imgNameList[i]);
	...
	private void shakeArray() {
		...
		bool[] barr = new bool[itemArr.Count];
    ...
    private void changTempCamVal()
    {
		...
		float radius1 = 2.5f * widthV / heightV;
		float s1 = radius1 * 2 / 10f;
		blackPlane.transform.localScale = new Vector3(s1,s1,s1);
		soldoutObj.transform.localScale = new Vector3(s1,s1,s1);
		// if(Size != 0)
		// {
